report 50052 "Statement of Account"
{
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = 'Report_Layouts/StatementofAccount.rdl';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            column(Address; ComRec.Address)
            {
            }
            column(Address_2; ComRec."Address 2")
            {
            }
            ...
            column(PDchequeDate; PDchequeDate)
            {
            }
            column(PDChequeNo; PDChequeNo)
            {
            }
            column(PDAmount; ABS(TotalPDAmount))
            {
            }
            column(PDChequeOption; PDChequeOption)
            {
            }
            column(PDChequeDocNo; PDChequeDocNo)
            {
            }
            column(Name_Customer; Customer.Name)
            {
            }
            column(Cus_Address; Customer.Address)
            {
            }
            ...
            column(BalLCY; Customer."Balance (LCY)")
            {
            }
            column(BalAsAtLCY; Customer."Balance As At (LCY)")
            {
            }
            column(DateFilter2; Customer."Date Filter 2")
            {
            }
            ...
            dataitem(CurrencyLoop; Integer)
            {
                ...
                dataitem(TempCustLedgEntryLoop; Integer)
                {
                    ...
                    column(CLEEndDateRemAmtLCY; RemaingBlanceLCY)
                    {
                        AutoFormatType = 1;
                    }
                    ...
                    column(CLEPostingDate; FORMAT(CustLedgEntryEndingDate."Document Date"))
                    {
                    }
                    ...
                    column(RemAmt_CLEEndDate; RemaingBlance)
                    {
                        AutoFormatExpression = CurrencyCode;
                        AutoFormatType = 1;
                    }
                    ...
                    column(UnallocatedAdvance; UnallocatedAdvance)
                    {
                    }
                    column(UnallocatedAdvanceLCY; UnallocatedAdvanceLCY)
                    {
                    }
                    ...
                    begin
                        ...
                        IF CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::Invoice THEN BEGIN
                            ...
                            RemaingBlance := CustLedgEntryEndingDate."Remaining Amount";
                            RemaingBlanceLCY := CustLedgEntryEndingDate."Remaining Amt. (LCY)";
                        END
                        ELSE BEGIN
                            RemaingBlance := 0;
                            RemaingBlanceLCY := 0;
                            UnallocatedAdvance += CustLedgEntryEndingDate."Remaining Amount";
                            UnallocatedAdvanceLCY += CustLedgEntryEndingDate."Remaining Amt. (LCY)";
                        END;
            ...
            begin
                //<Chin 20180111 1454
                CALCFIELDS("Balance As At (LCY)");
                //>
                PDchequeDate := 0D;
                PDAmount := 0;
                PDChequeDocNo := '';
                PDChequeNo := '';
                PDChequeOption := GenJnlLine."Document Type"::" ";
                ...
                /*
                GenJnlLine.RESET;
                GenJnlLine.SETRANGE("Journal Batch Name",'PD-CHEQUE');
                GenJnlLine.SETRANGE("Journal Template Name",'PD CHEQUE');
                GenJnlLine.SETRANGE("Account Type",GenJnlLine."Account Type"::Customer);
                GenJnlLine.SETRANGE("Account No.",Customer."No.");
                IF GenJnlLine.FINDFIRST THEN
                  REPEAT
                   PDchequeDate := GenJnlLine."Posting Date";
                   PDChequeOption := GenJnlLine."Document Type";
                   PDChequeDocNo := GenJnlLine."Document No.";
                   PDChequeNo := GenJnlLine."External Document No.";
                   PDAmount := GenJnlLine.Amount;
                  UNTIL GenJnlLine.NEXT=0;
                */
                TotalPDAmount := 0;
                CALCFIELDS("PD Check Amount");
                TotalPDAmount := Customer."PD Check Amount";
            ...
            begin
                ComRec.GET;
                //<Chin 20180111 1234
                SETRANGE("Date Filter 2", 0D, EndingDate);
                //MESSAGE('Date Set');
                //>
            end;
        }
    ...
    requestpage
    {
        ...
        begin
            ...
            PeriodLength3 := '<30D>';
            EVALUATE(PeriodLength, PeriodLength3);
            PrintAmountInLCY := TRUE;
            PrintDetails := TRUE;
            NewPagePercustomer := TRUE;
        end;
    }
    ...
    var
        ...
        EndingDate: Date;
        PDchequeDate: Date;
        PDChequeOption: Option " ",Payment,Invoice,"Credit Memo","Finance Charge Memo",Reminder,Refund;
        ...
        UnallocatedAdvance: Decimal;
        UnallocatedAdvanceLCY: Decimal;
        RemaingBlance: Decimal;
        RemaingBlanceLCY: Decimal;
        PDChequeNo: Code[30];
        PDAmount: Decimal;
        TotalPDAmount: Decimal;
        PDChequeDocNo: Code[20];
