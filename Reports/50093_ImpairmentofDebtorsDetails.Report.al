report 50093 "Impairment of Debtors Details"
{
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = 'Report_Layouts/ImpairmentofDebtorsDetails.rdl';
    Caption = 'Impairment of Debtors Details';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
                ...
                dataitem(TempCustLedgEntryLoop; Integer)
                {
                    ...
                    begin
                        ...
                        IF DetailedCustomerLedgerEntry.FINDSET(FALSE, FALSE) THEN
                            REPEAT
                                IF OperationDimension.GET('OPERATIONS', CustLedgEntryEndingDate."Global Dimension 2 Code") THEN
                                    OparationCode := OperationDimension.Name
                                ELSE
                                    OparationCode := '';
                                IF BUDimension.GET('BUSINESS UNITS', CustLedgEntryEndingDate."Global Dimension 1 Code") THEN
                                    BUCode := BUDimension.Name
                                ELSE
                                    BUCode := '';
            ...
            trigger OnAfterGetRecord()
            var
                CustLdgrEntry: Record "Cust. Ledger Entry";
                Cusbalance: Decimal;
            begin
                Cusbalance := 0;
                CustLdgrEntry.RESET;
                CustLdgrEntry.SETRANGE("Document Date", 0D, EndingDate);
                CustLdgrEntry.SETRANGE("Customer No.", "No.");
                CustLdgrEntry.SETRANGE(Open, TRUE);
                IF CustLdgrEntry.FINDFIRST THEN BEGIN
                    REPEAT
                        CustLdgrEntry.CALCFIELDS("Remaining Amt. (LCY)");
                        Cusbalance := Cusbalance + CustLdgrEntry."Remaining Amt. (LCY)";
                    UNTIL CustLdgrEntry.NEXT = 0;
                END;

                IF Cusbalance < 0 THEN
                    CurrReport.SKIP;
    ...
    var
        ...
        OperationDimension: Record "Dimension Value";
        BUDimension: Record "Dimension Value";
        CustFilter: Text;
