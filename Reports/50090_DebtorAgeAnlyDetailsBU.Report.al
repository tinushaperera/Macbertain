report 50090 "Debtor Age Anly Details- BU"
{
    ApplicationArea = All;
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = 'Report_Layouts/DebtorAgeAnlyDetailsBU.rdl';
    Caption = 'Debtor Age Analysis Details- By BU';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            dataitem(DataItem8503; "Cust. Ledger Entry")
            {
                ...
                RequestFilterFields = "Global Dimension 1 Code", "Global Dimension 2 Code", "Salesperson Code", "Customer No.", "Region Code", "Territory Code";
            ...
            dataitem(CurrencyLoop; Integer)
            {
                ...
                dataitem(TempCustLedgEntryLoop; Integer)
                {
                    ...
                    column(Credit_Limit; Customer."Credit Limit (LCY)")
                    {
                    }
                    column(Cus_Phone; Customer."Phone No.")
                    {
                    }
                    column(Business_Unit_Code; CustLedgEntryEndingDate."Global Dimension 1 Code")
                    {
                    }
                    column(Operation_code; CustLedgEntryEndingDate."Global Dimension 2 Code")
                    {
                    }
                    column(Sales_Person; CustLedgEntryEndingDate."Salesperson Code")
                    {
                    }
                    ...
                    column(Dimension_Name; DimensionCode.Name)
                    {
                    }
                    ...
                    column(PDCheque; PDCheque)
                    {
                    }
                    column(ChequeReturn; ChequeReturn)
                    {
                    }
                    column(GrandTotPayAmtLCY; GrandTotPayAmtLCY)
                    {
                    }
                    column(InvDescription; InvDescription)
                    {
                    }
                    column(InvDescription2; InvDescription2)
                    {
                    }
                    ...
                    begin
                        ...
                        IF DetailedCustomerLedgerEntry.FINDSET(FALSE, FALSE) THEN
                            REPEAT
                                IF DimensionCode.GET('BUSINESS UNITS', CustLedgEntryEndingDate."Global Dimension 1 Code") THEN;
                        ...
                        IF CustLedgEntryEndingDate."Applies-to ID" <> '' THEN
                            PDCheque := CustLedgEntryEndingDate."Amount to Apply";
                        ...
                        IF (CustLedgEntryEndingDate."Document Type" = CustLedgEntryEndingDate."Document Type"::Invoice) THEN BEGIN
                            ...
                            IF PostedSalesInv.FINDFIRST THEN
                                REPEAT
                                    Length := STRLEN(InvDescription + ' , ' + (PostedSalesInv.Description + ' ' + PostedSalesInv."Description 2"));
                                    IF Length < 1024 THEN
                                        InvDescription := InvDescription + ' , ' + (PostedSalesInv.Description + ' ' + PostedSalesInv."Description 2")
                                    ELSE
                                        InvDescription2 := InvDescription2 + ' , ' + (PostedSalesInv.Description + ' ' + PostedSalesInv."Description 2");
