report 50077 "Credit Note-After Posting"
{
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = 'Report_Layouts/CreditNoteAfterPosting.rdl';
                        ...
                        ELSE
                            ...
                            ELSE
                                IF ("Tax Area Code" = 'VAT15') THEN BEGIN
                                    CalCompTax.CalNBT("Tax Group Code", "Tax Area Code", Amount,
                                        NBTAmount);
                                    CalCompTax.CalVAT("Tax Group Code", "Tax Area Code", Amount, NBTAmount, VATAmount);
                                    SubTot := Amount + NBTAmount;
                                    IF (SubTot <> 0) AND (Quantity <> 0) THEN
                                        UnitPrice := SubTot / Quantity;
                                    NBTAmount := 0;
